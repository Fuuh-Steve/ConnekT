...
export const ProfilePage = ({ lookupBy }: { lookupBy?: string } = {}) => {
  ...
  // Listen for profile updates dispatched from other components (e.g. StudentDashboard)
  useEffect(() => {
    const refetch = async () => {
      if (!user) return;
      // Only refetch if this is the logged-in user's own profile
      if (username && username !== user.id && username !== profile?.username) return;
    ...
    window.addEventListener('profile_updated', refetch);
    return () => window.removeEventListener('profile_updated', refetch);
