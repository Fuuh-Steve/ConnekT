      ...
      if (error) {
        ...
        const mappedJobs: JobListing[] = data.map((job: any) => ({
          ...
          company: job.company && job.company !== 'ConnekT Partner'
            ? job.company
            : profileMap[job.recruiter_id] || job.company || t('fallbacks.companyNotProvided'),
    ...
    <div className="space-y-8">
      ...
      {loading ? (
        ...
        <div className="text-center py-20 bg-[rgb(var(--bg-side))] rounded-[3rem] border border-[rgb(var(--border))]">
          ...
          <p className="text-[rgb(var(--text-muted))]">{t('emptyState.subtitle')}</p>
        </div>
      )}
    </div>
  );
  ...
  >
    <div className="flex justify-between items-start mb-4">
      ...
      <div className="flex flex-col items-end gap-2">
        <div className="bg-[rgb(var(--accent))]/10 text-[rgb(var(--accent))] px-3 py-1.5 rounded-lg text-xs font-bold">
          <CountUp end={matchScore ?? 0} suffix={t('card.matchSuffix')} />
        </div>
      </div>
    </div>
    ...
    <div className="mt-auto pt-4 border-t border-[rgb(var(--border))] flex items-center justify-between">
      <span className="text-xs text-[rgb(var(--text-muted))] font-bold">
        <CountUp end={42} suffix={t('card.applicantsSuffix')} />
      </span>
      <Link href={`/jobs/${id}`} className="text-xs font-bold text-[rgb(var(--accent))] flex items-center gap-1">
        {t('card.details')} <ChevronRight className="w-3.5 h-3.5" />
      </Link>
    </div>
  </motion.div>
  );
